...
const Wishlist = () => {
    ...
    const removeFromWishlist = async (name, type) => {
        try {
            await axios.delete('http://localhost:3500/removeWishlist', {
                data : {
                    username: user.username,
                    name : name,
                    type : type
                    ...
                    <ul className="item-container">
                        {wishlist.map((wishListItem, index) => (
                            wishListItem.type === 'seed' ? (
                                ...
                                <FertilizerItem key={index} item={wishListItem} quantity={wishListItem.quantity} removeFromWishlist={removeFromWishlist}/>
                            ) : (
